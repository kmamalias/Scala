...
/**
  ...
  *
  *            SparkSQL join implemented
  */
object Joins7 {
  def main(args: Array[String]): Unit = {

    /** The "dictionary" of book abbreviations to full names */
    val abbrevsFile = "/home/hduser/spark-scala-tutorial/data/abbrevs-to-names.tsv"
    val abbrevs = Opt(
      name   = "abbreviations",
      value  = abbrevsFile,
    ...
    try {
      ...
      /* --This Join example is deprecated
      ...
      */
      ...
      val abbrevsDF = spark.createDataFrame(abbrevs)

      inputDF.createTempView("inputVW")
      abbrevsDF.createTempView("abbrevsVW")

      inputDF.cache
      abbrevsDF.cache

      val verses = spark.sql("Select t2._2 As Book, t1._2 As Text from inputVW t1 join abbrevsVW t2 on t1._1 = t2._1 Order by t2._1")

      verses.show(10, false)

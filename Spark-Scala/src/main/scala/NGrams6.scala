...
object NGrams6 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // to ensure that the final string is "\w+" and "\s+" for the regex.
    val ngramsRE = ngramsStr.replaceAll("%", """\\w+""").replaceAll("\\s+", """\\s+""").r   //The .r creates a RegEx Object
    ...
    try{
      ...
      val ngramz = sc.textFile(argz("input-path"))
        .flatMap{ line =>
          val text = TextUtil.toText(line)  //also converts to lowercase
